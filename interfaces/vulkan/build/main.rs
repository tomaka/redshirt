...
fn write_enum_values(mut out: impl Write, registry: &parse::VkRegistry) {
    ...
    for typedef in registry.type_defs.values() {
        match typedef {
            parse::VkTypeDef::Struct { fields } | parse::VkTypeDef::Union { fields } => {
...
fn write_commands_wrappers(mut out: impl Write, registry: &parse::VkRegistry) {
    for (command_id, command) in registry.commands.iter().enumerate() {
        ...
        writeln!(
            out,
            "    let mut msg_buf = Vec::<u8>::new();    // TODO: with_capacity"
        )
        .unwrap();
        ...
        writeln!(
            out,
            "    <u16 as Encode>::encode_to(&{}, &mut msg_buf);",
            command_id as u16
        )
        .unwrap();
        ...
        for (param_ty, param_name) in &command.params {
            write_serialize(
                ...
                &mut |fields| {
                    parse::gen_path_subfield_in_list(&command.params, registry, fields)
                        .map(|(n, ty)| (n, ty.clone()))
                        .unwrap()
                },
            );
        ...
        for (param_ty, param_name) in &command.params {
            ...
            let write_back = write_deserialize_response_into(
                param_ty,
                registry,
                &mut || {
                    let n = var_name;
...
fn write_redirect_handle(mut out: impl Write, registry: &parse::VkRegistry) {
    ...
    for (command_id, command) in registry.commands.iter().enumerate() {
        ...
        writeln!(
            out,
            "        {} => handle_{}(state, emitter_pid, msg_buf),",
    ...
    for command in &registry.commands {
        ...
        let f_ptr = match fpointers::command_ty(&command) {
            fpointers::CommandTy::Static => {
                format!("(state.static_pointers.r#{}.unwrap())", command.name)
            }
            fpointers::CommandTy::Instance => format!(
                "(state.instance_pointers.get(&{}).unwrap().r#{}.unwrap())",
        ...
        writeln!(
            out,
            "    assert!(msg_buf.is_empty(), \"Remaining: {{:?}}\", msg_buf.len());"
        )
        ...
        if command.name == "vkCreateInstance" {
            writeln!(
                out,
                "    state.assign_handle_to_pid(*{}, emitter_pid);",
                params_list[2]
            )
            .unwrap();
            ...
            writeln!(
                out,
                "        (state.get_instance_proc_addr)(*{}, name.as_ptr() as *const _)",
                params_list[2]
            )
            .unwrap();
            writeln!(out, "    }}));").unwrap();
            ...
            writeln!(
                out,
                "    state.instance_pointers.remove(&{});",
                params_list[0]
            )
            .unwrap();
            ...
            writeln!(
                out,
                "        let instance_ptrs = state.instance_pointers.get(&instance).unwrap();"
            )
            .unwrap();
            writeln!(
                out,
                "        let dev_get = instance_ptrs.vkGetDeviceProcAddr.unwrap();"
            )
            .unwrap();
            writeln!(
                out,
                "        DevicePtrs::load_with(|name: &std::ffi::CStr| {{"
            )
            .unwrap();
            writeln!(
                out,
                "            dev_get(*{}, name.as_ptr() as *const _)",
                params_list[3]
            )
            .unwrap();
            writeln!(out, "        }})").unwrap();
            ...
            writeln!(
                out,
                "            state.assign_handle_to_pid(*({}.offset(n as isize)), emitter_pid);",
                params_list[2]
            )
            .unwrap();
            writeln!(out, "        }}").unwrap();
            ...
            writeln!(
                out,
                "    for n in 0..(*{}).commandBufferCount {{",
                params_list[1]
            )
            .unwrap();
            writeln!(
                out,
                "        state.device_of_command_buffers.insert(*({}.offset(n as isize)), {});",
                params_list[2], params_list[0]
            )
            .unwrap();
            writeln!(
                out,
            ...
            writeln!(
                out,
                "    state.device_of_queues.insert(*{}, {});",
                params_list[3], params_list[0]
            )
            .unwrap();
        ...
        for ((param_ty, _), param_var) in command.params.iter().zip(params_list.iter()) {
            write_serialize(
                ...
                param_ty,
                registry,
                &mut |handle| format!("state.handles_host_to_vm.get(&{}).unwrap().1", handle),
                &mut |fields| {
                    if fields.len() >= 2 {
                        (format!("panic!()"), parse::VkType::Ident("int".to_string()))
                    // TODO:
                    } else {
                        let field_num = command
                            .params
                            .iter()
                            .position(|p| p.1 == fields[0])
                            .unwrap();
                        (
                            params_list[field_num].clone(),
                            command.params[field_num].0.clone(),
                        )
                    }
                },
            );
...
fn write_serialize(
    ...
    match (param_ty, type_def, skip_const) {
        ...
        (parse::VkType::Ident(ty_name), _, _) if ty_name == "size_t" => {
            ...
            writeln!(
                out,
                "        <Compact<u128> as Encode>::encode_to(&Compact({} as u128), &mut msg_buf);",
                param_name
            )
            .unwrap();
        }
        (parse::VkType::Ident(ty_name), _, _) if ty_name == "float" => writeln!(
            out,
            "        <u32 as Encode>::encode_to(&mem::transmute::<f32, u32>({}), &mut msg_buf);",
            param_name
        )
        .unwrap(),
        (parse::VkType::Ident(ty_name), _, _)
            if ty_name == "HANDLE"
                || ty_name == "HINSTANCE"
                || ty_name == "HWND"
                || ty_name == "LPCWSTR"
                || ty_name == "CAMetalLayer"
                || ty_name == "AHardwareBuffer" =>
        {
            // TODO: what to do here?
        }
        (parse::VkType::Ident(ty_name), _, _)
            if print_ty(param_ty).starts_with("*mut ")
                || print_ty(param_ty).starts_with("*const ")
                || print_ty(param_ty).contains("c_void") =>
        {
        ...
        | (parse::VkType::Ident(ty_name), None, false) => {
            writeln!(
                out,
                "        <{} as Encode>::encode_to(&{}, &mut msg_buf);",
                print_ty(param_ty),
                param_name
            )
            .unwrap();
        ...
        (parse::VkType::Ident(ty_name), Some(parse::VkTypeDef::DispatchableHandle), false) => {
            ...
            writeln!(
                out,
                "        <u32 as Encode>::encode_to(&({}), &mut msg_buf);",
                serialize
            )
            .unwrap();
        }
            ...
            for (field_ty, field_name) in fields {
                write_serialize(
                    ...
                    &mut |to_find| {
                        ...
                        let ty = parse::find_subfield_in_list(fields, registry, to_find)
                            .unwrap()
                            .clone();
                        (ret, ty)
                    },
        ...
        | (parse::VkType::ConstPointer(ty_name, _), _, _)
            if print_ty(&param_ty).contains("void") =>
        {
        ...
        | (parse::VkType::ConstPointer(ty_name, parse::VkTypePtrLen::One), _, false) => {
            ...
            writeln!(
                out,
                "            <u32 as Encode>::encode_to(&1, &mut msg_buf);"
            )
            .unwrap();
            write_serialize(
                out,
            ...
            writeln!(
                out,
                "            <u32 as Encode>::encode_to(&0, &mut msg_buf);"
            )
            .unwrap();
            writeln!(out, "        }}").unwrap();
        }
        ...
        | (parse::VkType::ConstPointer(ty_name, parse::VkTypePtrLen::NullTerminated), _, false) => {
            ...
            writeln!(
                out,
                "            let len = (0isize..).find(|n| *{}.offset(*n) == 0).unwrap() as u32;",
                param_name
            )
            .unwrap();
            writeln!(
                out,
                "            <u32 as Encode>::encode_to(&len, &mut msg_buf);"
            )
            .unwrap();
            ...
            write_serialize(
                out,
                false,
                &format!("(*{}.offset(n as isize))", param_name),
                &ty_name,
                registry,
                serialize_handles,
                other_field,
        ...
        (
            parse::VkType::MutPointer(
                ty_name,
                parse::VkTypePtrLen::OtherField {
                    before_other_field,
                    other_field: field,
                    after_other_field,
                },
        ...
        | (
            ...
            writeln!(
                out,
                "            let len = {}{}{};",
                before_other_field, other_field_name, after_other_field
            )
            .unwrap();
            writeln!(
                out,
                "            <u32 as Encode>::encode_to(&(len as u32), &mut msg_buf);"
            )
            .unwrap(); // TODO: remove?
            ...
            write_serialize(
                out,
                false,
                &format!("(*{}.offset(m as isize))", param_name),
                &ty_name,
                registry,
                serialize_handles,
                other_field,
...
fn write_deserialize(
    ...
    match (ty, type_def) {
        ...
        (parse::VkType::Ident(ty_name), _) if ty_name == "size_t" => format!(
            "<Compact<u128> as Decode>::decode(&mut msg_buf)?.0 as {}",
            print_ty(ty)
        ),
        ...
        (parse::VkType::Ident(ty_name), _)
            if ty_name == "HANDLE" || ty_name == "HINSTANCE" || ty_name == "HWND" =>
        {
            // TODO: what to do here?
            format!("{{ let v: {} = panic!(); v }}", print_ty(ty))
        }
        (parse::VkType::Ident(ty_name), _)
            if print_ty(ty).starts_with("*mut ")
                || print_ty(ty).starts_with("*const ")
                || print_ty(ty).starts_with("c_void") =>
        {
            format!(
                "<Compact<u128> as Decode>::decode(&mut msg_buf)?.0 as usize as {}",
                print_ty(ty)
            )
        }
        ...
        (parse::VkType::Ident(ty_name), Some(parse::VkTypeDef::Struct { fields })) => {
            ...
            for (field_ty, field_name) in fields {
                let field_expr = write_deserialize(field_ty, registry, &mut |e, mutable| {
                    interm_step_gen(e, mutable)
                });
                let field_interm = interm_step_gen(
                    format!("/* {}::{} */ {}", ty_name, field_name, field_expr),
                    false,
                );
        ...
        | (parse::VkType::ConstPointer(ty_name, _), _)
            if print_ty(ty).contains("void") =>
        {
            // TODO: what to do here?
            format!("mem::zeroed::<{}>()", print_ty(ty))
        }
        ...
        | (parse::VkType::ConstPointer(ty_name, parse::VkTypePtrLen::One), _) => {
            let is_mut = if let parse::VkType::MutPointer(_, _) = ty {
                true
            } else {
                false
            };
            let is_present = interm_step_gen(
                "<u32 as Decode>::decode(&mut msg_buf)? != 0".to_owned(),
                false,
            );

            let interm = {
                let inner = write_deserialize(&ty_name, registry, &mut |f, mutable| {
                    let var = interm_step_gen(
                        format!("if {} {{ Some({}) }} else {{ None }}", is_present, f),
                        mutable,
                    );
                    if mutable {
            ...
            if is_mut {
                format!(
                    "{}.as_mut().map(|p| p as *mut _).unwrap_or(ptr::null_mut())",
                    var
                )
            } else {
                format!(
                    "{}.as_ref().map(|p| p as *const _).unwrap_or(ptr::null())",
                    var
                )
            }
        }
        ...
        | (parse::VkType::ConstPointer(ty_name, parse::VkTypePtrLen::NullTerminated), _) => {
            ...
            let len_var = interm_step_gen(
                format!(
                    "/* len({}) */ <u32 as Decode>::decode(&mut msg_buf)? as usize",
                    print_ty(ty_name)
                ),
                false,
            );
            ...
            format!(
                "if !{var}.is_empty() {{ {var}.as_ptr() }} else {{ ptr::null() }}",
                var = var
            )
        }
        ...
        | (parse::VkType::ConstPointer(ty_name, parse::VkTypePtrLen::OtherField { .. }), _) => {
            ...
            if is_mut {
                format!(
                    "if !{var}.is_empty() {{ {var}.as_mut_ptr() }} else {{ ptr::null_mut() }}",
                    var = var
                )
            } else {
                format!(
...
fn write_deserialize_response_into(
    ...
    match (ty, type_def, force_write) {
        ...
        (parse::VkType::Ident(ty_name), _, true) if ty_name == "size_t" => {
            // A `size_t` is platform-specific, but by using the `Compact` we can make it the same size everywhere.
            format!(
                "{} = <Compact<u128> as Decode>::decode(&mut msg_buf)?.0 as usize;",
                out_var_name
            )
        }
        (parse::VkType::Ident(ty_name), _, true) if ty_name == "float" => format!(
            "{} = mem::transmute::<u32, f32>(<u32 as Decode>::decode(&mut msg_buf)?);",
            out_var_name
        ),
        ...
        | (parse::VkType::Ident(ty_name), Some(parse::VkTypeDef::Bitmask), true)
        | (parse::VkType::Ident(ty_name), Some(parse::VkTypeDef::NonDispatchableHandle), true)
        | (parse::VkType::Ident(ty_name), None, true) => format!(
        ...
        (parse::VkType::ConstPointer(ty_name, parse::VkTypePtrLen::One), _, _) => {
            write_deserialize_response_into(
                ty_name,
                registry,
                var_name_assign,
                &format!("(*{})", out_var_name),
        ...
        (parse::VkType::Array(ty_name, len), _, _) => {
            ...
            let inner = write_deserialize_response_into(
                ty_name,
                registry,
                var_name_assign,
                &format!("{}[{}]", out_var_name, iter_var),
                force_write,
            );
            if inner.is_empty() {
                String::new()
        ...
        | (parse::VkType::MutPointer(ty_name, parse::VkTypePtrLen::OtherField { .. }), _, _) => {
            ...
            let inner = write_deserialize_response_into(
                ...
                registry,
                var_name_assign,
                &format!("(*({}.offset({} as isize)))", out_var_name, iter_var),
                true,
            );
            format!(
                "{{ \
...
fn write_get_instance_proc_addr(mut out: impl Write, registry: &parse::VkRegistry) {
    ...
    for command in &registry.commands {
        ...
        writeln!(
            out,
            "            let ptr = wrapper_{} as unsafe extern \"system\" fn({}) -> {};",
            command.name,
            params_tys,
            print_ty(&command.ret_ty)
        )
        .unwrap();
        writeln!(
            out,
            "            mem::transmute::<_, PFN_vkVoidFunction>(ptr)"
        )
        .unwrap();
        writeln!(out, "        }}").unwrap();
    }
...
fn print_ty(ty: &parse::VkType) -> String {
    match ty {
        ...
        parse::VkType::ConstPointer(t, _)
            if **t == parse::VkType::Ident("SECURITY_ATTRIBUTES".into()) =>
        {
        ...
        parse::VkType::MutPointer(t, _) if **t == parse::VkType::Ident("void".into()) => {
            "*mut c_void".to_owned()
        }
        parse::VkType::ConstPointer(t, _) if **t == parse::VkType::Ident("void".into()) => {
            "*const c_void".to_owned()
        }
