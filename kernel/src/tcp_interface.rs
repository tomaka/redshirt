    ...
    pub fn handle_message(&mut self, event_id: Option<u64>, message: tcp::ffi::TcpMessage) {
        match message {
            ...
            tcp::ffi::TcpMessage::Read(read) => {
                let event_id = event_id.unwrap();
                self.sockets.get_mut(&read.socket_id).unwrap().start_read(event_id);
            },
            tcp::ffi::TcpMessage::Write(write) => {
                let event_id = event_id.unwrap();
                self.sockets.get_mut(&write.socket_id).unwrap().start_write(event_id, write.data);
            },
        }
    }
...
impl TcpConnec {
    pub fn start_read(&mut self, event_id: u64) {
        let pending_read = match self {
            TcpConnec::Socket { ref mut pending_read, .. } => pending_read,
            _ => panic!()
        };
    ...
    pub fn start_write(&mut self, event_id: u64, data: Vec<u8>) {
        let pending_write = match self {
            TcpConnec::Socket { ref mut pending_write, .. } => pending_write,
            _ => panic!()
        };
    ...
    pub fn next_event<'a>(&'a mut self) -> impl Future<Output = TcpResponse> + 'a {
        future::poll_fn(move |cx| {
            let (new_self, event) = match self {
                ...
                TcpConnec::Socket { socket_id, tcp_stream, pending_read, pending_write } => {
                    let write_finished = if let Some((msg_id, data_to_write)) = pending_write {
                        if !data_to_write.is_empty() {
                            let num_written = ready!(AsyncWrite::poll_write(Pin::new(tcp_stream), cx, &data_to_write)).unwrap();
                            for _ in 0..num_written { data_to_write.remove(0); }
                        }
                        if data_to_write.is_empty() {
                    ...
                    if let Some(msg_id) = write_finished {
                        *pending_write = None;
                        return Poll::Ready(TcpResponse::Write(msg_id, tcp::ffi::TcpWriteResponse {
                            result: Ok(())
                        }));
                    }
                    ...
                    if let Some(msg_id) = pending_read.clone() {
                        let mut buf = [0; 1024];
                        let num_read = ready!(AsyncRead::poll_read(Pin::new(tcp_stream), cx, &mut buf)).unwrap();
                        *pending_read = None;
                        return Poll::Ready(TcpResponse::Read(msg_id, tcp::ffi::TcpReadResponse {
