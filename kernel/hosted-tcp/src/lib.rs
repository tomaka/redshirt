    ...
    TMsgId: Send + 'static,
{
    ...
    pub async fn handle_message(
        ...
        message: nametbd_network_interface::ffi::TcpMessage,
    ) {
        let mut sockets = self.sockets.lock().await;

        match message {
            nametbd_network_interface::ffi::TcpMessage::Open(open) => {
            ...
            nametbd_network_interface::ffi::TcpMessage::Listen(listen) => {
                let message_id = message_id.unwrap(); // TODO: don't unwrap; but what to do?
                let socket_addr = {
            ...
            nametbd_network_interface::ffi::TcpMessage::Close(close) => {
                let _ = sockets.remove(&close.socket_id);
            }

            nametbd_network_interface::ffi::TcpMessage::Accept(accept) => {
                let message_id = message_id.unwrap(); // TODO: don't unwrap; but what to do?
                sockets
            ...
            nametbd_network_interface::ffi::TcpMessage::Read(read) => {
                let message_id = message_id.unwrap(); // TODO: don't unwrap; but what to do?
                sockets
            ...
            nametbd_network_interface::ffi::TcpMessage::Write(write) => {
                let message_id = message_id.unwrap(); // TODO: don't unwrap; but what to do?
                sockets
        ...
        match message {
            BackToFront::OpenOk {
                ...
                *front_state = FrontSocketState::Connected(sender);
                ...
                (
                    open_message_id,
                    nametbd_network_interface::ffi::TcpOpenResponse {
                        result: Ok(socket_id),
                    }
                    .encode(),
                )
            ...
            BackToFront::OpenErr {
                ...
                (
                    open_message_id,
                    nametbd_network_interface::ffi::TcpOpenResponse { result: Err(()) }.encode(),
                )
            }
            ...
            BackToFront::ListenOk {
                ...
                *front_state = FrontSocketState::Listener(sender);
                ...
                (
                    listen_message_id,
                    nametbd_network_interface::ffi::TcpListenResponse {
            ...
            BackToFront::ListenErr {
                ...
                (
                    listen_message_id,
                    nametbd_network_interface::ffi::TcpListenResponse { result: Err(()) }.encode(),
                )
            }
            ...
            BackToFront::Accept { message_id, socket } => {
                ...
                (
                    message_id,
                    nametbd_network_interface::ffi::TcpAcceptResponse {
            ...
            BackToFront::Read { message_id, result } => (
                message_id,
                nametbd_network_interface::ffi::TcpReadResponse { result }.encode(),
            ),

            BackToFront::Write { message_id, result } => (
                message_id,
                nametbd_network_interface::ffi::TcpWriteResponse { result }.encode(),
            ),
        }
    }
}
