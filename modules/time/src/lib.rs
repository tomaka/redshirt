...
use self::ParseError::{
    InvalidDay, InvalidDayOfMonth, InvalidDayOfWeek, InvalidDayOfYear, InvalidFormatSpecifier,
    InvalidHour, InvalidMinute, InvalidMonth, InvalidSecond, InvalidSecondsSinceEpoch, InvalidTime,
    InvalidYear, InvalidZoneOffset, MissingFormatConverter, UnexpectedCharacter,
};
...
mod tests {
    use super::ParseError::{
        InvalidFormatSpecifier, InvalidTime, InvalidYear, MissingFormatConverter,
    };
    use super::{
        at, at_utc, get_time, precise_time_ns, precise_time_s, strptime, Duration, ParseError,
        PreciseTime, SteadyTime, Timespec,
    };

    use std::mem;
    use std::sync::{LockResult, Mutex, MutexGuard, Once, ONCE_INIT};
    ...
    fn test_strftime() {
        ...
        assert_eq!(
            local.strftime("%G").unwrap().to_string(),
            "2009".to_string()
        );
        ...
        assert_eq!(local.strftime("%p").unwrap().to_string(), "PM".to_string());
        assert_eq!(
            local.strftime("%R").unwrap().to_string(),
            "15:31".to_string()
        );
        assert_eq!(
            local.strftime("%r").unwrap().to_string(),
        ...
        assert_eq!(
            local.strftime("%s").unwrap().to_string(),
            "1234567890".to_string()
        );
        assert_eq!(
            local.strftime("%T").unwrap().to_string(),
            "15:31:30".to_string()
        );
        ...
        assert_eq!(
            local.strftime("%v").unwrap().to_string(),
        ...
        assert_eq!(
            local.strftime("%X").unwrap().to_string(),
            "15:31:30".to_string()
        );
        // FIXME (#2350): support locale
        assert_eq!(
        ...
        assert_eq!(
            local.strftime("%Y").unwrap().to_string(),
            "2009".to_string()
        );
        ...
        assert_eq!(
            local.strftime("%z").unwrap().to_string(),
            "-0800".to_string()
        );
        assert_eq!(
            local.strftime("%+").unwrap().to_string(),
        ...
        assert_eq!(
            local.rfc822z().to_string(),
            "Fri, 13 Feb 2009 15:31:30 -0800".to_string()
        );
        assert_eq!(
            local.rfc3339().to_string(),
            "2009-02-13T15:31:30-08:00".to_string()
        );
        ...
        assert_eq!(
            utc.rfc822().to_string(),
            "Fri, 13 Feb 2009 23:31:30 GMT".to_string()
        );
        assert_eq!(
            utc.rfc822z().to_string(),
