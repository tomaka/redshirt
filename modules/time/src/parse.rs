use super::{at_utc, ParseError, Timespec, Tm, NSEC_PER_SEC};
...
fn parse_type(s: &mut &str, ch: char, tm: &mut Tm) -> Result<(), ParseError> {
    match ch {
        ...
        'F' => parse_type(s, 'Y', tm)
            ...
            .and_then(|()| parse_char(s, '-'))
            .and_then(|()| parse_type(s, 'd', tm)),
        'H' => match match_digits_in_range(s, 1, 2, false, 0, 23) {
            Some(v) => {
                tm.tm_hour = v;
                Ok(())
            }
        ...
        'I' => match match_digits_in_range(s, 1, 2, false, 1, 12) {
            Some(v) => {
        ...
        'j' => match match_digits_in_range(s, 1, 3, false, 1, 366) {
            Some(v) => {
                tm.tm_yday = v - 1;
                Ok(())
            }
        ...
        'k' => match match_digits_in_range(s, 1, 2, true, 0, 23) {
            Some(v) => {
                tm.tm_hour = v;
                Ok(())
            }
        ...
        'l' => match match_digits_in_range(s, 1, 2, true, 1, 12) {
            Some(v) => {
                tm.tm_hour = if v == 12 { 0 } else { v };
                Ok(())
            }
        ...
        'M' => match match_digits_in_range(s, 1, 2, false, 0, 59) {
            Some(v) => {
                tm.tm_min = v;
                Ok(())
            }
        ...
        'm' => match match_digits_in_range(s, 1, 2, false, 1, 12) {
            Some(v) => {
                tm.tm_mon = v - 1;
                Ok(())
            }
        ...
        's' => match match_digits_i64(s, 1, 18, false) {
            Some(v) => {
        ...
        'S' => match match_digits_in_range(s, 1, 2, false, 0, 60) {
            Some(v) => {
                tm.tm_sec = v;
                Ok(())
            }
        ...
        //'s' {}
        'T' | 'X' => parse_type(s, 'H', tm)
            .and_then(|()| parse_char(s, ':'))
            .and_then(|()| parse_type(s, 'M', tm))
            .and_then(|()| parse_char(s, ':'))
            .and_then(|()| parse_type(s, 'S', tm)),
        't' => parse_char(s, '\t'),
        'u' => match match_digits_in_range(s, 1, 1, false, 1, 7) {
            Some(v) => {
        ...
        'v' => parse_type(s, 'e', tm)
            ...
            .and_then(|()| parse_type(s, 'Y', tm)),
        //'W' {}
        'w' => match match_digits_in_range(s, 1, 1, false, 0, 6) {
            Some(v) => {
                tm.tm_wday = v;
                Ok(())
            }
        ...
        'Y' => match match_digits(s, 4, 4, false) {
            Some(v) => {
                tm.tm_year = v - 1900;
                Ok(())
            }
        ...
        'y' => match match_digits_in_range(s, 1, 2, false, 0, 99) {
            Some(v) => {
                tm.tm_year = v;
                Ok(())
            }
        ...
        'z' => {
            if parse_char(s, 'Z').is_ok() {
                ...
                let sign = if parse_char(s, '+').is_ok() {
                    1
                } else if parse_char(s, '-').is_ok() {
                    -1
                } else {
                    return Err(ParseError::InvalidZoneOffset);
                };
