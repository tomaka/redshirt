    ...
    TIfId: Clone + Hash + PartialEq + Eq,
{
    ...
    pub fn inject_interface_data(&mut self, id: &TIfId, data: impl AsRef<[u8]>) {
        self.devices.get_mut(id).unwrap()   // TODO: don't unwrap
    ...
    pub async fn next_event<'a>(&'a mut self) -> NetworkManagerEvent<'a, TIfId> {
        // TODO: optimize?
        let next_event = future::select_all(self.devices.iter_mut().map(move |(n, d)| Box::pin(d.next_event().map(move |ev| (n.clone(), ev)))));
